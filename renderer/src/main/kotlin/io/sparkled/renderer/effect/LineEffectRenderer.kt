...
class LineEffectRenderer : EffectRenderer() {
    ...
    fun render(ctx: RenderContext) {
        val startLed = 0
        val endLed = ctx.getChannel().getLedCount() - 1
    ...
    internal fun renderLine(ctx: RenderContext, startLed: Int, endLed: Int, mirror: Boolean) {
        ...
        val length = ParamUtils.getIntegerValue(ctx.getEffect(), ParamName.LENGTH)

        val progress = if (mirror) 1 - ctx.getProgress() else ctx.getProgress()
