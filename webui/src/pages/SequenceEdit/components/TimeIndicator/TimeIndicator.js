...
class TimeIndicator extends Component {
  ...
  render() {
    ...
    const seconds = this.renderSeconds(durationFrames, framesPerSecond);
    return (
      <svg className="time-indicator" style={{ width }}>
        ...
        <pattern id="quarter-second-pattern" width={PIXELS_PER_FRAME * (framesPerSecond / 4)} height="10" patternUnits="userSpaceOnUse">
          <line x1="0" y1="3" x2="0" y2="6"/>
        </pattern>

        <pattern id="half-second-pattern" width={PIXELS_PER_FRAME * (framesPerSecond / 2)} height="10" patternUnits="userSpaceOnUse">
  ...
  renderSeconds(durationFrames, framesPerSecond) {
