...
class SongListPage extends Component {
  ...
  getFilteredSongs = () =>
    _.filter(this.props.songs || [], this.songMatchesSearch)
  ...
  songMatchesSearch = song => {
    ...
    return (
      _.filter([name, album, artist], field =>
        _.includes(field.toLowerCase(), searchQuery)
      ).length > 0
    )
  }
}
...
SongListPage = connect(
  mapStateToProps,
  { setCurrentPage, showAddModal, fetchSongs }
)(SongListPage)
