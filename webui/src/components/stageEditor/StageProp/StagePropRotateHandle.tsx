...
const StagePropRotateHandle: React.FC<Props> = props => {
  ...
  if (rotateHandle === null) {
    ...
    stagePropRotateHandle.on('pointerdown', (event: InteractionEvent) =>
    ...
    rotateHandle
      .on('pointermove', (event: InteractionEvent) =>
        onDragMove(event, props, rotateHandle, dragState)
      )
...
function onDragMove(
  ...
  let newRotation =
    Math.atan2(originY - relativeParentY, originX - relativeParentX) -
    quarterTurn
